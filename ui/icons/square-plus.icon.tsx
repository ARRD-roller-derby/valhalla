/**
 *
 * @description Font Awesome Free 6.4.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc.
 */

interface SquarePlusIconProps {
  className?: string
}
export function SquarePlusIcon({ className }: SquarePlusIconProps) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 448 512" className={className}>
      <path d="M48 80V432H400V80H48zM0 32H48 400h48V80 432v48H400 48 0V432 80 32zM200 368V344 280H136 112V232h24 64V168 144h48l0 24v64h64 24v48H312 248v64 24H200z" />
    </svg>
  )
}
