/**
 *
 * @description Font Awesome Free 6.4.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc.
 */

interface PipeIconProps {
  className?: string
}
export function PipeIcon({ className }: PipeIconProps) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 64 512" className={className}>
      <path d="M64 0V32 480v32H0V480 32 0H64z" />
    </svg>
  )
}
