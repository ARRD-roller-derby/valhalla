import { S3_BUCKET, S3_ENDPOINT, S3_KEY, S3_PRIVATE, S3_REGION } from './constants'
import {
  S3Client,
  PutObjectCommand,
  CreateBucketCommand,
...
export default class S3 {
  ...
  async deleteMedia(link: string) {
    const Key = link.replace(`${process.env.S3_PUBLIC_ENDPOINT || ''.replace('BUCKETNAME', this.bucketName)}/`, '')
    try {
      await this.s3Client.send(
